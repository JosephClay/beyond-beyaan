...
{
	public class BasicShieldTechnology
	{
		...
		public Dictionary<string, object>[] Activate(int origX, int origY, int destX, int destY, int maxX, int maxY, Dictionary<string, object> itemValues)
		{
			...
			{
				itemValues.Add("DamageAbsorption", float.Parse((string)itemValues["baseDamageAbsorption"]));
			}
		...
		public Dictionary<string, object>[] OnHit(int impactX, int impactY, int shipX, int shipY, int shipSize, Dictionary<string, object> itemValues, Dictionary<string, object> particleValues)
		{
			...
			if (itemValues.ContainsKey("shieldAbsorption") && particleValues.ContainsKey("damage"))
			{
				values[0]["damage"] = (float)particleValues["damage"] - (float)itemValues["shieldAbsorption"];
			...
			string particleName = "particle" + ((shipSize / 2) + (shipSize % 2));
			
			if (itemValues.ContainsKey(particleName))
			{
				values[1].Add("SpawnParticle", itemValues[particleName]);
			}
		...
		{
			if (value.Key.StartsWith("particle"))
			{
				return AddValue(value.Key, value.Value, values);
			}
			switch (value.Key)
			{
				...
				case "baseDamageAbsorption": return AddValue("shieldAbsorption", float.Parse((string)value.Value), values);
				...
				case "baseDamageAbsorptionMultiplier": return MultiplyValue("shieldAbsorption", float.Parse((string)value.Value), values);
		...
		{
			if (equipmentValues.ContainsKey("shieldAbsorption"))
			{
				shipValues["shieldAbsorption"] = (float)shipValues["shieldAbsorption"] + (float)equipmentValues["shieldAbsorption"];
			}

...
{
	public class BasicStellarEngineTechnology
	{
		...
		private Dictionary<string, object> HandleValue(KeyValuePair<string, object> value, Dictionary<string, object> values)
		{
			switch (value.Key)
			{
				...
				case "baseGalaxySpeed": return AddValue("galaxySpeed", float.Parse((string)value.Value), values);
				...
				case "baseGalaxySpeedMultiplier": return MultiplyValue("galaxySpeed", float.Parse((string)value.Value), values);
		...
		{
			if (equipmentValues.ContainsKey("galaxySpeed"))
			{
				shipValues["galaxySpeed"] = (float)shipValues["galaxySpeed"] + (float)equipmentValues["galaxySpeed"];
			}

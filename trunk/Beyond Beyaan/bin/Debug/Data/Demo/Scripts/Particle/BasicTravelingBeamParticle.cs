...
{
	public class BasicTravelingBeamParticle
	{
		public Dictionary<string, object> Spawn(Dictionary<string, object> itemValues)
		{
			...
			itemValues.Add("SpawnLength", float.Parse((string)itemValues["spawnLength"], System.Globalization.CultureInfo.InvariantCulture));
		...
		public Dictionary<string, object>[] Update(Dictionary<string, object> itemValues, float frameDeltaTime)
		{
			...
			{
				if (!itemValues.ContainsKey("LifeTime"))
				{
					itemValues.Add("LifeTime", 1.0f);
				}
				else
				{
					itemValues["LifeTime"] = (float)itemValues["LifeTime"] - frameDeltaTime;
					if ((float)itemValues["LifeTime"] <= 0)
					{
